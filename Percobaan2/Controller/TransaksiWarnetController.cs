...
namespace Percobaan2.Controller
{
    public class TransaksiWarnetController
    {
        ...
        public List<TransaksiWarnet> ReadByDateRange(DateTime startDate, DateTime endDate)
        {
            ...
            {
                _repository = new TransaksiWarnetRepository(context);
                transaksiWarnets = _repository.ReadByDateRange(startDate, endDate);
            }
        ...
        {
            int totalHarga = _repository.CalculateTotalHarga(startDate, endDate);

            // Display or use the totalHarga as needed
